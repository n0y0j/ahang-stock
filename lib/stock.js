...
const getHistoricalData = async (ticker) => {
    ...
    table.map((index, item) => {
        var temp = new Date($(item).find("td:nth-child(1)").text())
        const date = temp.setDate(temp.getDate() + 1)

        historicalData.push({
            date: new Date(date),
            open: parseFloat($(item).find("td:nth-child(2)").text()),
            high: parseFloat($(item).find("td:nth-child(3)").text()),
            low: parseFloat($(item).find("td:nth-child(4)").text()),
            close: parseFloat($(item).find("td:nth-child(5)").text()),
            adj_close: parseFloat($(item).find("td:nth-child(6)").text()),
            volume: parseInt($(item).find("td:nth-child(7)").text().replace(/,/gi, "")),
        })
    })
